<?php
    ...
    while($row=mysqli_fetch_array($result_group_id))
    {
        ...
        {
            $data[]=  array(  'week_start'=>$row['ts_week_start'], 'revision_count'=>$row['revision_count']  ) ;
        }

        $json_group_id[]= $current_group_id ;
        $json_group_id_data[]=  array('group_id'=> $current_group_id , 'data'=> $data) ;

    }



    print json_encode( array('dates'=>$json_Dates, 'group_id'=>$json_group_id,'data'=>$json_group_id_data) );
