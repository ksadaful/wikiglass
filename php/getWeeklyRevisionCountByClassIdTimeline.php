<?php
    ...
    while($row=mysqli_fetch_array($result_group_id))
    {
        ...
        {
            $data[]=  array( 'x' => $row['ts_week_start'], 'y'=> $row['revision_count'], 'url' => '#/class/'.$classID."/".$params[1]) ;
        }

        $json_group_id[]= $current_group_id ;
        $json_group_id_data[]=  array('name'=> 'Group '.$params[1]  , 'data'=> $data) ;

    }



    print json_encode( array('data'=>$json_group_id_data) , JSON_UNESCAPED_SLASHES );
